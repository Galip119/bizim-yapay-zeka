    ...
    if sorgu or dosya_icerigi:
        ...
        with st.spinner("Eymen-GPT düşünüyor..."):
            try:
                ...
                else: kullanici_mesaji += "Soru: Lütfen yüklediğim bu dosyayı analiz et."
                ...
                if not basarili_oldu: st.error("Tüm modellerin kotası dolmuş veya bir bağlantı hatası var.")
                    ...
                    match = re.search(r'<(?:dusunce|düşünce)>(.*?)</(?:dusunce|düşünce)>', ham_cevap, re.DOTALL | re.IGNORECASE)
                    if match:
                        dusunce_blogu = match.group(1).strip()
                        temiz_cevap = re.sub(r'<(?:dusunce|düşünce)>.*?</(?:dusunce|düşünce)>', '', ham_cevap, flags=re.DOTALL | re.IGNORECASE).strip()

                    st.session_state.mesaj_gecmisi.append({"role": "assistant", "content": temiz_cevap})
                    with st.chat_message("assistant"):
                        if dusunce_blogu:
                            with st.expander("🧠 Eymen-GPT'nin Düşünme Adımlarını Göster"): st.write(dusunce_blogu)
...
# ==========================================
    ...
    resim_sorgu = st.text_input("Neyin resmini çizmek istersin?", key=f"resim_input_{st.session_state.form_num}")
...
# ==========================================
    ...
    sesli_sorgu = st.text_input("Ne duymak istersin?", key=f"sesli_input_{st.session_state.form_num}")
...
# ==========================================
    ...
    st.write("Sınırsız ses aralığı (C1'den B7'ye), 10 farklı enstrüman ve çok daha uzun şarkılar!")
    ...
    if uret_butonu and muzik_sorgu:
        with st.spinner("Eymen-GPT devasa stüdyoyu kuruyor ve notaları yazıyor... (Bu biraz sürebilir)"):
            try:
                # Sınırsız Ses Aralığı Sözlüğü (C1 - B7)
                ...
                # LLM'den 10 farklı kanal istiyoruz (Sadece 16 adımlık bir kalıp isteyelim ki JSON bozulmasın, Python ile onu uzatacağız)
                sistem_mesaji = """Sen çılgın bir müzik dehasısın. Kullanıcının verdiği hisse göre tam 16 adımlık bir ana müzik iskeleti oluştur.
                SADECE GEÇERLİ JSON FORMATINDA YAZ.
                {
                    "tempo": 13
